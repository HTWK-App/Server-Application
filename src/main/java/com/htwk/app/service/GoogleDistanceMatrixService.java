...
public class GoogleDistanceMatrixService {
  ...
  public Map<Long, Building> getDistances(String location, List<Building> buildings)
      throws Exception {
    String destinations = "";
  ...
  private String getGoolgeDistance(String location, String destinations) {
    ...
    ResponseEntity<String> response =
        restTemplate.exchange(distanceUri, HttpMethod.GET, new HttpEntity<Object>(headers),
            String.class);
    ...
    if (status.equalsIgnoreCase("OK")) {
      ...
      if (rows.size() >= 0) {
        ...
        for (index = 0; index < elements.size(); index++) {
          ...
          try {
            long distance =
                elements.get(index).getAsJsonObject().get("distance").getAsJsonObject()
                    .get("value").getAsLong();
            ...
            logger.error("invalid entry in distanceResponse");
          }
        }
      }
    }
  ...
  private Map<Long, Building> getShortestDistance(String content, List<Building> buildings)
      throws Exception {
    JsonObject response = parser.parse(content).getAsJsonObject();
    String status = response.get("status").getAsString();
    ...
    if (status.equalsIgnoreCase("OK")) {
      ...
      if (rows.size() >= 0) {
        ...
        for (index = 0; index < elements.size(); index++) {
          ...
          long distance =
              elements.get(index).getAsJsonObject().get("distance").getAsJsonObject().get("value")
                  .getAsLong();
          // wrong maappping
